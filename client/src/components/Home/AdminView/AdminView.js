...
const AdminView = () => {
  ...
  useEffect(() => {
    fetch("https://localhost:5000/hotelviewrequest", {
      method: "GET",
      mode: "cors",
      credentials: "include",
    })
      ...
      .then((data) => {
        console.log("Hotel : ",data.ownership);
        if (data.status !== "ok") {
          SetValid(false);
          setConnected(true);
        }
        if (data.ownership !== "hotel") {
          SetValid(false);
          setConnected(true);
        }
        setConnected(true);
        console.log(data)
        if (data.ownership === "hotel" && data.status === "ok") {
          var ob = { ad: "as" };
          var dr = data.data;
  ...
  function handleClick(id) {
    fetch("https://localhost:5000/hoteldelete", {
      method: "POST",
      mode: "cors",
            ...
            <Table className={classes.table} aria-label="customized table">
              <TableHead>
                <TableRow>
                  ...
                  <StyledTableCell align="right">Adult</StyledTableCell>
