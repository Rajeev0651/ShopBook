...
const AdminView = () => {
  ...
  useEffect(() => {
    fetch("https://localhost:5000/saloonviewrequest", {
      method: "GET",
      mode: "cors",
      credentials: "include",
    })
      ...
      .then((data) => {
        console.log("Saloon : ",data.ownership);
        if (data.status !== "ok") {
          SetValid(false);
          setConnected(true);
        }
        if (data.ownership !== "saloon") {
          SetValid(false);
          setConnected(true);
        }
        setConnected(true);
        console.log(data)
        if (data.ownership === "saloon" && data.status === "ok") {
          var ob = { ad: "as" };
          var dr = data.data;
  ...
  function handleClick(id) {
    fetch("https://localhost:5000/saloondelete", {
      method: "POST",
      mode: "cors",
  ...
  return (
    <React.Fragment>
      {valid === false ? (
        ...
        <div>
          <h1 align="center">Booking Saloon record</h1>
